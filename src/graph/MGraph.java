...
// 1) MGraph extends class Graph, where Graph is class for general graph.
...
// are sub-graphs of general graph, for example:
//		Graph g = {{a, b, c, d, e}, {(a, b), (b, c), (a, c), (c, d), (d, e)}}
// 		MGraph mg = {{A, B}, {(A, B)}}, where A = {{a, b, c}, {(a, b), (b, c)}} 
// 		and B = {{d, e}, {(d, e)}}, and nw(A) = 3 and nw(B) = 2;
// 4) MGraph will additionally maintain edges of subgraphs of nodes;
public class MGraph extends Graph{
	...
	public void setNode(final MGraph oriG) {
		...
		for (int i = 0; i < nNode; i++) {
			...
			//nodeInNode[i] = new TreeSet<Integer>();
			//Iterator<Integer> iterN = oriG.nodeInNode[i].iterator();
			//while (iterN.hasNext()) {
			//	nodeInNode[i].add(new Integer((Integer)iterN.next()));
			//}
			edgeInNode[i] = new TreeSet<Edge>();
			edgeInNode[i].addAll(oriG.edgeInNode[i]);
//			Iterator<Edge> iterE = oriG.edgeInNode[i].iterator();
//			while (iterE.hasNext()) {
//				edgeInNode[i].add(new Edge((Edge)iterE.next()));
//			}
		}
	}
	
	public void setEdge(final TreeMap<MEdge, Double> edge) {
		nodeInd = null; nodeInd = new int[nNode + 1];
		Arrays.fill(nodeInd, -1);
		nodeInd[0] = 0;
		
		nEdge = edge.size();
		edgeT = null; edgeT = new int[nEdge];
		w = null; w = new double[nEdge];
		oriS = null; oriS = new int[nEdge];
		oriT = null; oriT = new int[nEdge];
		
		int ind = 0;
		Iterator<MEdge> iter = edge.keySet().iterator();
		while (iter.hasNext()) {
			MEdge e = (MEdge) iter.next();
	...
	public MGraph merge(final int cid[], final int edgeCid[], final double _nodeW[],
			final TreeMap<Integer, WST> edgeCC[]) {
...
//		System.out.print("(" + nNode + " " + compCnt + " " + compNEdge + ")\t");
		...
		for (int i = 0; i < nNode; i++) {
			...
			for (int j = nodeInd[s]; j < nodeInd[s + 1]; j++) {
				...
				if (s > t) continue;
				if (edgeCid[j] != -1) {
					resultG.edgeInNode[cidMap.get(cid[edgeCid[j]])].add(new Edge(oriS[j], oriT[j]));
				}
			}
		}
		...
		for (int i = 0; i < nNode; i++) {
			if (cid[i] != i) continue;
			Iterator<Entry<Integer, WST>> iter = edgeCC[i].entrySet().iterator();
			while (iter.hasNext()) {
				Entry<Integer, WST> entry = (Entry<Integer, WST>)iter.next();
				if (cid[entry.getKey()] == entry.getKey()) {
					WST wst = entry.getValue();
					MEdge me1 = new MEdge(cidMap.get(i), cidMap.get(entry.getKey()), 
							wst.getS(), wst.getT());
					MEdge me2 = new MEdge(cidMap.get(entry.getKey()), cidMap.get(i),  
							wst.getS(), wst.getT());
					resultGEdge.put(me1, wst.getW());
					resultGEdge.put(me2, wst.getW());
				}
			}
		}
	...
	public void checkNodeW(Graph ag) {
		...
		System.out.println("sum of edge weight: " + ag.getSumOfEdgeW(edgeInNode[rootID]));
...
//		System.out.println("mis:" + mis + "\t");
	...
	public void testGraph(final HashMap<Integer, Integer> id2name) {
		...
		for (int i = 0; i < nNode; i++) {
			System.out.print(i + ": ");
			for (int j = nodeInd[i]; j < nodeInd[i + 1]; j++) {
				System.out.print(edgeT[j] + " " + w[j] + "(" + id2name.get(oriS[j]) + " "
						+ id2name.get(oriT[j]) + ")\t");
			}
			System.out.println();
		}
