...
public class LinkCutTree {
	 ...
	 * @param nNode the number of nodes (trees)
	 */
	...
	public LinkCutTree(LinkCutTree lct) {
		int nNode = lct.left.length;
		left = new int[nNode];
		System.arraycopy(lct.left, 0, left,0, nNode);
		right = new int[nNode];
		System.arraycopy(lct.right, 0, right,0, nNode);
		parent = new int[nNode];
		System.arraycopy(lct.parent, 0, parent,0, nNode);
		pathParent = new int[nNode];
		System.arraycopy(lct.pathParent, 0, pathParent,0, nNode);
		costToParent = new double[nNode];
		System.arraycopy(lct.costToParent, 0, costToParent,0, nNode);
		realParent = new int[nNode];
		System.arraycopy(lct.realParent, 0, realParent,0, nNode);
		pathNode = new int[nNode];
		pathNodeWeight = new double[nNode];
	}
	
	/**
	 * return a boolean value indicating whether v is the root of a tree
	 * @param v vertex
	...
	/**
	 * find the root of the tree to which v belongs
	 * @param v vertex
	 ...
	 * @param v root of a tree
	 * @param w arbitrary node
	 * @param cost the cost of linking operation
	 */
	...
	/**
	 * cut the link from v to v's parent if v is not a root
	 * @param v vertex 
	 */
	 ...
	 * access(v) first and change the direction of path from root down to v
	 * @param v vertex
	 */
