...
public class SpanningTree {
	...
	public static MGraph doMinimumST(MGraph g) {
		...
		ArrayList<MEdge> selectEdge = new ArrayList<MEdge>(nEdge);
		final int[] oriS = g.getOriS();
		final int[] oriT = g.getOriT();
		for (int i = 0; i < nEdge / 2; i++) {
			if (eid[i] == -1) continue;
			MEdge e1 = new MEdge(s[i], t[i], oriS[eid[i]], oriT[eid[i]], w[i]);
			MEdge e2 = new MEdge(t[i], s[i], oriS[eid[i]], oriT[eid[i]], w[i]);
			selectEdge.add(e1);
			selectEdge.add(e2);
		}
		Collections.sort(selectEdge);
